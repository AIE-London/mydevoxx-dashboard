  ...
  it("should work return data and handles a 404", () => {
    return normalSetup() // Setup mock for positive test
      ...
      .then(
        ...
        error => {
          // Should not error, so thro expect error below
          if (isTestingException(error)) {
            // Let it flow down to the catch as-is
            throw error;
          } else {
            // Error on the getSlots call for normal setup
            throw new UnexpectedErrorException(
              'Unexpected error when retrieving day schedule after "Normal" setup',
              error
            ); // Throw an expect error to differentiate from 404 in the catch
          }
        }
      )
      ...
      .then(
        ...
        error => {
          if (isTestingException(error)) {
            ...
            throw new MappingSetupException(
              'Mapping setup failed for "notFoundSetup"',
              error
            );
          }
        }
      ...
      .then(
        results => {
          throw new UnexpectedSuccessException(
            'Unexpected success when retrieving day schedule after "Not Found" setup'
          ); // Should not get here, so fail with expect error
        },
        error => {
          if (isTestingException(error)) {
            // Let it flow down to the catch as-is
            ...
            if (error.body && error.body.toString() === "Not Found") {
              // Expected error from negative test
              expect(error.statusCode).toBe(404);
            } else {
              throw UnexpectedErrorException(
                'Unexpected error when retrieving day schedule after "Not Found" setup',
                error
              );
            }
          }
      ...
      .then(
        ...
        error => {
          if (isTestingException(error)) {
            ...
            throw new MappingSetupException(
              'Mapping setup failed for "emptyArraySetup"',
              error
            );
          }
        }
      ...
      .then(
        ...
        error => {
          ...
          if (isTestingException(error)) {
            ...
            throw new UnexpectedErrorException(
              'Unexpected error when retrieving day schedule after "Empty Array" setup',
              error
            ); // Throw an expect error to differentiate from 404 in the catch
          }
        }
