...
describe("getSpeakers", () => {
  it("should work return data and handles a 404", () => {
    return normalSetup() // Setup mock for positive test
      .then(speakers.getSpeakers, error => {
        raiseOrPassError(
          "MappingSetupException",
      ...
      .then(
        ...
        error => {
          // Should not error, so throw exception
          raiseOrPassError(
            "UnexpectedErrorException",
            'Unexpected error when retrieving speakers after "Normal" setup',
            error
          );
        }
      )
      ...
      .then(
        results => {
          // Should not pass, so throw exception
          raiseOrPassError(
            "UnexpectedSuccessException",
            'Unexpected success when retrieving speakers after "Not Found" setup'
          );
        },
        error => {
          expectNotFoundOrRethrowError(
            error,
            'Unexpected error on "getSpeakers" after "notFoundSetup"'
          );
        }
      )
      .catch(error => {
