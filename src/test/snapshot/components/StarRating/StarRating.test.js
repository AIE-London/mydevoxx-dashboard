...
test("StarRating with rating 5", () => {
  const tree = renderer.create(<StarRating rating="5" />).toJSON();
  expect(tree).toMatchSnapshot();
});
...
test("StarRating with rating 3", () => {
  const tree = renderer.create(<StarRating rating="3" />).toJSON();
  expect(tree).toMatchSnapshot();
});
test("StarRating with rating 2", () => {
  const tree = renderer.create(<StarRating rating="2" />).toJSON();
  expect(tree).toMatchSnapshot();
});
test("StarRating with rating 1", () => {
  const tree = renderer.create(<StarRating rating="1" />).toJSON();
  expect(tree).toMatchSnapshot();
});
