...
test("Review Component - Fully populated review", () => {
  const tree = renderer.create(<Review review={fullyPopReview} />).toJSON();
  expect(tree).toMatchSnapshot();
});

test("Review Component - Empty Review", () => {
  const tree = renderer.create(<Review review={emptyReview} />).toJSON();
  expect(tree).toMatchSnapshot();
});

test("Review Component - Missing Image", () => {
  const tree = renderer.create(<Review review={reviewWithoutImage} />).toJSON();
  expect(tree).toMatchSnapshot();
});
